...
while(cap.isOpened()):
    ...
    if ret==True:
        ...
        canny = cv2.Canny(frame,80,500,3)
        ...
        for i in range(0,len(contours)):
            ...
            approx = cv2.approxPolyDP(contours[i],cv2.arcLength(contours[i],True)*0.06,True)
            ...
            if(not(abs(cv2.contourArea(contours[i]))<200 or not(cv2.isContourConvex(approx)))):
                ...
                if(len(approx) == 3):
                    ...
                    if(vtc==4):
                        ...
                        # display radius size of enclosing circle
                        (x, y), radius = cv2.minEnclosingCircle(contours[i])
                        center = (int(x), int(y))
                        radius = int(radius)
                        cv2.circle(frame, center, radius, (0,255,0),2)
                        cv2.putText(frame, str(radius),(int(x),int(y)),cv2.FONT_HERSHEY_SIMPLEX,scale,(255,0,0),2,cv2.LINE_AA)
                    elif(vtc==6):
